...
with open("storage/prompts/themes.yaml", encoding="utf-8") as f:
...
def get_theme_description(theme_key: str) -> str:
    return PROMPTS["themes"].get(theme_key, {}).get("description", f"univers magique {theme_key}")

def substitute_prompt_placeholders(prompt: str, **kwargs) -> str:
...
def generate_image(word: str, theme_key: str, index: int, prompt=None):
    if prompt is None:
        ...
        # Apply template substitution to custom prompts
        theme_desc = get_theme_description(theme_key)
        prompt = substitute_prompt_placeholders(prompt,
                                              word=word,
                                              theme=theme_key,
...
def generate_video(image_path: str, word: str, theme_key: str, index: int, motion_prompt=None):
    if motion_prompt is None:
        ...
        # Apply template substitution to custom motion prompts
        motion_prompt = substitute_prompt_placeholders(motion_prompt,
                                                     word=word,
                                                     theme=theme_key)
...
def generate_theme_music(theme_key: str, theme_name_fr: str, music_prompt=None, lyrics=None, languages=["fr"]):
    ...
    for lang in languages:
        ...
        if lyrics is None:
            ...
            lyrics_prompt = f"Génère des paroles de chanson joyeuses pour enfants en français sur le thème '{theme_name_fr}' utilisant ces mots: {', '.join(words[:6])}. Maximum 600 caractères. Structure simple: [verse 1] ... [chorus] ... [verse 2] ... Respond ONLY with a valid JSON object in this exact format: {{\"lyrics\": \"[verse 1]\\n...\"}}. Do not include any other text, explanations, or formatting."
    ...
    if lyrics is None:
        ...
        # Générer lyrics
        lyrics_prompt = f"Génère des paroles de chanson joyeuses pour enfants en français sur le thème '{theme_name_fr}' utilisant ces mots: {', '.join(words[:6])}. Maximum 600 caractères. Structure simple: [verse 1] ... [chorus] ... [verse 2] ... [chorus]"

        lyrics_output = replicate.run("meta/llama-2-70b-chat", input={"prompt": lyrics_prompt, "temperature": 0.8, "max_tokens": 600})
...
def generate_image_draft(prompt, path):
    """Generate image for drafts - saves to provided path"""
    output = replicate.run(
        "recraft-ai/recraft-v3",
        input={"prompt": prompt, "width": 1024, "height": 1024, "output_format": "png"}
    )
...
def generate_video_draft(image_path, video_path, prompt):
    """Generate video for drafts - saves to provided video_path as .webm"""
    print(f"  → Animation SILENCIEUSE de '{video_path}'...")
    ...
    video_url = output
    temp_path = video_path.replace('.webm', '_temp.mp4')
    ...
    # Conversion en WebM
    subprocess.run([
        "ffmpeg", "-i", temp_path, "-c:v", "libvpx-vp9", "-an", video_path
    ], check=True)

    # Supprimer temp
    os.remove(temp_path)

    print(f"    Vidéo muette générée : {video_path}")
    return video_path
...
def get_dominant_color(image_path):
    try:
        ...
        if isinstance(most_common, tuple) and len(most_common) == 3:
            return f"#{most_common[0]:02x}{most_common[1]:02x}{most_common[2]:02x}{most_common[2]:02x}"
        else:
